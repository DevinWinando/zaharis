@extends('layout.resepsionisLayout')

@section('title', 'Admin - Kamar')

@push('style')
    <link rel="stylesheet" href="{{ asset('css/admin/kamar.css') }}">
    <link rel="stylesheet" href="{{ asset('vendors/simple-datatables/style.css') }}" />
@endpush

@section('main')
<div class="d-flex justify-content-between">
    <h3>Data Kamar</h3>
    <div>
        <button class="btn btn-primary d-print-none" onclick="window.print()">Print</button>
    </div>
</div>
<div class="table-responsive mt-4">
    <table class="table table-borderless table-hover" id="tabel-reservasi">
        <thead>
            <tr>
                <th class="text-center">Nomor</th>
                <th class="text-center" class="text-center">Pemesan</th>
                <th class="text-center" class="text-center">Nomor Kamar</th>
                <th class="text-center">Tanggal Mulai</th>
                <th class="text-center">Tanggal Selesai</th>
                <th class="text-center">Status</th>
                <th class="text-center">Harga Total</th>
                <th class="text-center">Proses</th>
            </tr>
        </thead>
        <tbody>
            @foreach($reservasi as $item)
                <tr class="text-center">
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $item->client->nama }}</td>
                    <td>{{ $item->kamar->nomor }}</td>
                    <td class="text-bold-500">{{ $item->mulai }}</td>
                    <td >{{ $item->selesai }}</td>
                    <td>{{ $item->status }}</td>
                    <td>{{ $item->harga_total }}</td>
                    <td>
                        <form action="" method="post">
                            @csrf
                            <select name="status" id="" class="form-select" onchange="this.form.submit()">
                                <option value="" selected hidden disabled>Pilih status</option>
                                <option value="diproses">Diproses</option>
                                <option value="dipesan">Dipesan</option>
                                <option value="dibatalkan">Dibatalkan</option>
                                <option value="selesai">Selesai</option>
                            </select>

                            <input type="hidden" name="idReservasi" value="{{ $item->id }}">
                            <input type="hidden" name="idKamar" value="{{ $item->kamar->id }}">
                        </form>
                    </td>
                </tr>
            @endforeach
        </tbody>
    </table>
</div>
@endsection

@push('script')

    <script src="{{ asset('vendors/simple-datatables/simple-datatables.js') }}"></script>
    <script>
        const tabel = document.querySelector('#tabel-reservasi');
        const dataTable = new simpleDatatables.DataTable(tabel)
    </script>

    @if (session('success'))
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: '{{ session('success') }}',
            })

        </script>
        
    @endif

    @if (session('error'))
        <script>
            Swal.fire({
                icon: 'Error',
                title: 'Error',
                text: '{{ session('error') }}',
            })
        </script>
    @endif
@endpush
